...
public class SQLASTOutputVisitor extends SQLASTVisitorAdapter implements ParameterizedVisitor, PrintableVisitor {
    ...
    static {
        defaultPrintStatementAfterSemi = true;//getBoolean(System.getProperties(), "druid.sql.output.printStatementAfterSemi"); // compatible for early versions
    }

    protected final Appendable appender;
    private String indent = "  ";              // 填充前缀字符 , \t - TAB键
    private int indentCount = 0;
    private boolean prettyFormat = true;       // 是否多行格式化， false 仅一行,true 多行格式化
    protected boolean ucase = true;            // 关键字大小写  true-大写,false-小写
    protected int selectListNumberOfLine = 5;  // 列排出5列才换行

    protected boolean groupItemSingleLine = false; // 是否分组每行一个字段,true-是,false-否
    protected int exprGroupOfLine = 3;             // 当groupItemSingleLine=false时才有效，指分组多少个字段才换行
	...
	public int getSelectListNumberOfLine() {
		return selectListNumberOfLine;
	}

	public void setSelectListNumberOfLine(int selectListNumberOfLine) {
		this.selectListNumberOfLine = selectListNumberOfLine;
	}
	...
	public int getExprGroupOfLine() {
		return exprGroupOfLine;
	}

	public void setExprGroupOfLine(int exprGroupOfLine) {
		this.exprGroupOfLine = exprGroupOfLine;
	}
    ...
    public boolean visit(SQLBinaryOpExpr x) {
        ...
        for (int i = groupList.size() - 1; i >= 0; --i) {
            ...
            if (relational) {
            	...
            	if ((exprCount % exprGroupOfLine == 0 ) && !((i==0) && (right instanceof SQLExistsExpr))){  // liqj 2017-07-30 on 条件 exprGroupOfLine组才换行
            		println();
            	}else{
            		print(' ');
            	}
        	...
        	if ((i==0) && (right instanceof SQLExistsExpr) ) 
        		println();
    	...
    	//if (x.getRight() instanceof SQLExistsExpr) println();
    ...
    public boolean visit(SQLCaseExpr x) {
        ...
        for (int i = 0, size = x.getItems().size(); i < size; ++i) {
            if((i+1)%exprGroupOfLine==0) println(); else print(' ');
            ...
            for (int i = 0; i < itemSize; ++i) {
                ...
                if (i != 0) {
                    if (groupItemSingleLine) {
                        ...
                        print(", ");
                        if (groupFiled%selectListNumberOfLine ==0){
                        	println("");
                        }
                    }
                }
    ...
    public boolean visit(SQLSelectQueryBlock x) {
        ...
        if (x.getFrom() != null) {
        	if ((x.getParent()!=null)&&(x.getParent().getParent()!=null)&&(x.getParent().getParent() instanceof SQLExistsExpr)){
        		print(' ');
        	}else{
        		println();
        	}
