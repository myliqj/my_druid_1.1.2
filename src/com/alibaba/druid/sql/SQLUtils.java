...
public class SQLUtils {
    public static void main(String[] args){
    	String sql = "/*afdffdfdd*/ -- dfd \nselect /*afdffdfdd*/ s.*,s.aa ,1  b,  'dfa' bb from aaa s -- adfd \n where /*afdfd*/   a='' and xx=1 and exists(select * from a)";
    	//System.out.println(SQLUtils.format(sql, "db2"));
//    	db2
//    	oracle
//    	sqlserver
//    	mysql
...
//    	hsql
    	...
    	String sql1 ="select  a.card,'export to ' concat trim(a.tabname)\n" + 
    			"  concat '.del of del messages ex_del.log select * from '\n" + 
    			"  concat trim(a.tabschema) concat '.' concat trim(a.tabname) concat ''\n" + 
    			"from syscat.tables a\n" + 
    			"where tabschema = 'FSSB' and type = 'T'\n" + 
    			"  and a.tabname not like 'EXPLAIN%' and a.tabname like 'S%'\n" + 
    			"  and a.tabname not like 'SJJHJK%'  and a.tabname not like 'STA%'\n" + 
    			"  and a.tabname not like 'SIR%'\n" + 
    			"  --and a.tabname not like 'SEC%'\n" + 
    			"order by a.card \n";
    ...
    public static String format_custom_1_1_2(String sql, String params) {
        try {
        	String[] opts = params.split("/");
        	// 参数：0类型/1是否关键字大写/2是否格式化多行/3是否保持注释/4分组是否一行/5字段分行数量/6表达式分行数量/7前缀字符
        	String dbType = opts[0];
        	boolean isUpCase = "1".equals(opts[1]);
        	boolean isFormat = "1".equals(opts[2]);
        	boolean isKeepComments = "1".equals(opts[3]);
        	boolean groupItemSingleLine = "1".equals(opts[4]);
        	int fieldLineCount = Integer.valueOf(opts[5]);
        	int exprLineCount = Integer.valueOf(opts[6]);
        	String indent = opts[7];
        	
        	FormatOption option = new FormatOption(isUpCase,isFormat);
        	option.setKeepComments(isKeepComments);
        	option.setFieldLineCount(fieldLineCount);
        	option.setExprLineCount(exprLineCount);
        	option.setIndent(indent);
        	option.setGroupItemSingleLine(groupItemSingleLine);
        	
            SQLStatementParser parser = SQLParserUtils.createSQLStatementParser(sql, dbType, option.isKeepComments);
            parser.setKeepComments(option.isKeepComments);
    ...
    public static String toSQLString(List<SQLStatement> statementList
        ...
        visitor.setSelectListNumberOfLine(option.getFieldLineCount());
        visitor.setGroupItemSingleLine(option.isGroupItemSingleLine());
        visitor.setExprGroupOfLine(option.getExprLineCount());
        visitor.setIndent(option.getIndent());
    ...
    public static class FormatOption {
        ...
        private boolean isKeepComments = true;
    	private int fieldLineCount = 5;
    	private boolean groupItemSingleLine = false; // 是否每个分组一行
    	private int exprLineCount = 3;               // 表达式分行数量 
    	private String indent = "  ";

        public boolean isGroupItemSingleLine() {
			return groupItemSingleLine;
		}

		public void setGroupItemSingleLine(boolean groupItemSingleLine) {
			this.groupItemSingleLine = groupItemSingleLine;
		}

		public boolean isKeepComments() {
			return isKeepComments;
		}

		public void setKeepComments(boolean isKeepComments) {
			this.isKeepComments = isKeepComments;
		}

		public int getFieldLineCount() {
			return fieldLineCount;
		}

		public void setFieldLineCount(int fieldLineCount) {
			this.fieldLineCount = fieldLineCount;
		}

		public int getExprLineCount() {
			return exprLineCount;
		}

		public void setExprLineCount(int groupLineCount) {
			this.exprLineCount = groupLineCount;
		}
